...
if length(handles.model.labels.class_labels) > 0
    class_labels = model.labels.class_labels;
else
    for g=1:max(model.settings.class); class_labels{g} = ['class ' num2str(g)]; end
end
k = 0;
str_disp = {};
% variables ID
...
if strcmp(model.type,'plsda')
    for p = 1:size(model.b,2)
        k = k + 1;
        str_disp{k} = ['coefficients - ' class_labels{p}];
        store_for_plot{k}.val = model.b(:,p);
        lab = (['coefficients - ' class_labels{p}]);
...
if strcmp(model.type,'simca') || strcmp(model.type,'uneq')
    for g = 1:max(model.settings.class)
        for p = 1:size(model.T{g},2)
            k = k + 1;
            str_disp{k} = ['[' class_labels{g} '] loadings on PC ' num2str(p)];
            store_for_plot{k}.val = model.modelpca{g}.L(:,p);
            lab = ([class_labels{g} ' - loadings on PC ' num2str(p) ' - EV = ' num2str(round(model.modelpca{g}.exp_var(p)*10000)/100) '%']);
            store_for_plot{k}.lab = lab;
            store_for_plot{k}.pred = [];
        end
    end
...
if strcmp(model.type,'lda') | strcmp(model.type,'pcalda') && strcmp(handles.showspecificmodel,'none')
    ...
    for p = 1:size(model.Lstd,2)
        ...
        store_for_plot{k}.val = model.Lstd(:,p);
...
if (strcmp(model.type,'pcalda') && strcmp(handles.showspecificmodel,'pca')) || strcmp(model.type,'pcaqda') && strcmp(handles.showspecificmodel,'pca')
    ...
    for p = 1:size(model.settings.modelpca.L,2)
        ...
        store_for_plot{k}.val = model.settings.modelpca.L(:,p);
        lab = (['loadings on PC ' num2str(p) ' - EV = ' num2str(round(model.settings.modelpca.exp_var(p)*10000)/100) '%']);
        store_for_plot{k}.lab = lab;
        store_for_plot{k}.pred = [];
    end
end
