...
elseif strncmp(Lab_Tx,'output weights',14)
    classin = x - 1;
    thrc = handles.model.settings.thr(classin);
    line([thrc thrc],y_lim,'Color','r','LineStyle',':')   
end
...
elseif strncmp(Lab_Ty,'output weights',14)
    classin = y - 1;
    thrc = handles.model.settings.thr(classin);
    line(x_lim,[thrc thrc],'Color','r','LineStyle',':')
end
...
% make plots
if (strcmp(handles.model.type,'pf') || strcmp(handles.model.type,'svm') || strcmp(handles.model.type,'lda') || strcmp(handles.model.type,'pcalda') || strcmp(handles.model.type,'backprop')) && strcmp(handles.showspecificmodel,'none')
...
if strcmp(model.type,'backprop')
    % output weights
    for g=1:size(model.output_pred,2)
        k = k + 1;
        str_disp{k} = ['outputs - ' class_labels{g}];
        store_for_plot{k}.val = model.output_pred(:,g);
        store_for_plot{k}.lab = ['outputs - ' class_labels{g}];
        store_for_plot{k}.pred = [];
        if isstruct(pred)
            store_for_plot{k}.pred = pred.output_pred(:,g);
        end
    end
end
