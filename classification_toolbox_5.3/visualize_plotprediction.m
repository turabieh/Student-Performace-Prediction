...
elseif strcmp(model.type,'knn') || strcmp(model.type,'simca') || strcmp(model.type,'uneq') || strcmp(model.type,'backprop') % if model was calibrated on scaled data
    X = model.settings.raw_data;
...
elseif strcmp(model.type,'simca')
    ...
    for g=1:max(model.settings.class)
        P{g} = reshape(pred.binary_assignation(:,g), [length(x) length(y)]);
    end
...
elseif strcmp(model.type,'pf')
    for g=1:length(model.settings.Xclass)
        contour(xx, yy, P{g}, [model.settings.thr(g) model.settings.thr(g)], 'LineColor', col_ass(g+1,:), 'LineWidth', 2);
    end
